...
const CreateSubCategory = (props) => {
  ...
  const handleSubmit=(e)=>{
      ...
      createSub(name,parentId,user.token).then((res)=>{
          setname("")
          setChecked(prev=>!prev)
        //   props.rerender()
    ...
    <>
      ...
      <div>
        ...
        <select onChange={(e)=>setParentId((e.target.value))}>
          <option disabled selected hidden>Select Parent Category</option>
      ...
      <span className="sub-list pr-4 " onClick={()=>setListCheck(prev =>!prev)}><i className="fas fa-list pl-2" style={{fontSize:"2rem"}}></i></span>
